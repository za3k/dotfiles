...
elseif hostname == "juice" then
end

-- TODO: awful.widget.watch
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ }, "XF86AudioRaiseVolume", function ()
	   ...
	   {description = "Raise the volume 9%", group="volume"}),
...
clientkeys = awful.util.table.join(
    ...
    -- Custom
    -- Windows-[comma, period] switch virtual tag sets one level
...
never = function() return awful.tag.find_by_name(awful.screen.focused(), "-") end
always = function() return awful.tag.find_by_name(awful.screen.focused(), "=") end
isStreamMode = function () 
    return #(always():clients()) > 0
end
globalkeys = awful.util.table.join(globalkeys,
    awful.key({ modkey }, "-", function () never():view_only() end),
    awful.key({ modkey, "Shift" }, "-", function () 
        if client.focus then
            client.focus:move_to_tag(never())
        end
    end),
    awful.key({ modkey }, "=", function () always():view_only() end),
    awful.key({ modkey, "Shift" }, "=", function () 
        if client.focus then
            client.focus:move_to_tag(always())
        end
    end),
    awful.key({ modkey }, "/", function () 
        -- always():swap(never()) # Nope! This swaps the order, not the clients.
        local always = always()
        local never = never()
        local alwaysClients = always:clients()
        local neverClients = never:clients()
        for i, x in pairs(alwaysClients) do
            x:move_to_tag(never)
        end
        for i, x in pairs(neverClients) do
            x:move_to_tag(always)
        end
    end)
)
...
function stringify(t)
  if type(t) == "table" then
    local s = tostring(t) .. "[" .. tostring(#t) .. "] {"
    for i,x in pairs(t) do
        s = s .. " " .. tostring(i) .. "=" .. tostring(x)
    end
    return s .. " }"
  else
    return tostring(t)
...
function reshuffle_always()
    local alwaysClients = always():clients()

    -- Move "always" clients on the lowest-number IDs.
    -- This is globally, not just what's visible now.
    local isAlways = {}
    for i, c in ipairs(client.get()) do  isAlways[c] = false end
    for i, c in ipairs(alwaysClients) do isAlways[c] = true  end
    local seq = {} -- true, true, false, false etc series of whether things are the sidebar
    for i, x in ipairs(client.get()) do
        seq[i] = isAlways[x]
    end

    --dbg("isAlways", isAlways)
    --dbg("Seq", seq)
    local alwaysSeen = 0
    for i, c in ipairs(client.get()) do
        if isAlways[c] then -- Bubble it down
            alwaysSeen = alwaysSeen + 1
            local pos = i
            while pos > alwaysSeen do -- Swap down one
    ...
    -- Set to tile.left so new clients get added on the left
end
        ...
        awful.key({ modkey }, "#" .. i + 9,
                  function ()
                        ...
                        if isStreamMode() then
                           ...
                           -- Put "always" clients on the right, which is master windows for the 2-column tile layout
                           awful.layout.set(awful.layout.suit.tile.left)
                           tag.master_width_factor = 0.15

                           -- Allocate the correct number of master windows
                           tag.master_count = #always():clients()

                           -- Make sure the "always" clients are in the lowest-numbered slots
                           reshuffle_always(screen)
                        else
                           tag:view_only()
                        end
                  end,
...
always():connect_signal("tagged", function(t, c)
    local screen = c.screen
    if (not awesome.startup) and #screen.tags > 1 then
      c:toggle_tag(always())
      reshuffle_always()
    end
end)
